...
st.set_page_config(page_title="ERP 14.0 MIRROR REPORT", layout="wide", page_icon="📊")
...
# --- NOVO MOTOR DE PDF (REFLEXO DA TELA) ---
def gerar_pdf_espelho(df_filtrado, metrics, period):
    ...
    # 2. Quadro de Métricas (Igual ao Topo da Tela)
    elements.append(Paragraph("1. RESUMO FINANCEIRO", style_h2))
    m_data = [
        ["TOTAL CONTRATADO", "ATINGIMENTO META", "JÁ EM CAIXA", "SALDO A RECEBER"],
    ...
    # 3. Saúde Financeira (O gráfico de barras em formato de tabela)
    ...
    # 4. Detalhamento de Contratos e Parciais
    ...
    # Ordenamos por data base para refletir a tela
...
if menu == "📊 Dashboard Analytics" and not df.empty:
    ...
    # (Motor de lançamento mantido igual à 13.1)
    st.info("Acesse a aba de lançamentos para novos registros.")