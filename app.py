...
# --- 1. CONFIGURAÇÕES MESTRAS (SEUS LINKS E IDS) ---
...
if menu == "📝 Lançar Venda":
    ...
    with st.form("venda_blindada", clear_on_submit=True):
        ...
        f_parc = c3.number_input("Parcelas (0 = À Vista)", min_value=0, value=0, step=1)
        
        if st.form_submit_button("🚀 GRAVAR NA PLANILHA"):
            if not f_cli or f_total <= 0:
                st.error("Preencha o Cliente e o Valor Total.")
            else:
                success_count = 0
                
                def enviar_dados(tipo, valor, venc):
                    payload = {
                        f"entry.{IDs['cliente']}": str(f_cli).strip(),
                        f"entry.{IDs['vendedor']}": str(f_vend).strip(),
                        f"entry.{IDs['tipo']}": str(tipo),
                        f"entry.{IDs['vencimento']}": venc.strftime('%Y-%m-%d'), # Formato internacional para o Form
                ...
                if f_parc == 0:
                    # VENDA À VISTA
                    if enviar_dados("À Vista", f_total, f_data):
                        success_count += 1
                else:
                    # VENDA PARCELADA
                    # 1. Entrada
                    if f_entrada > 0:
                        if enviar_dados("Entrada", f_entrada, f_data):
                            success_count += 1
                    ...
                    for i in range(int(f_parc)):
                        # Se teve entrada, 1ª parcela em 30 dias. Se não, começa na data base.
                        data_venc = f_data + relativedelta(months=i+1 if f_entrada > 0 else i)
                        if enviar_dados(f"Parc {i+1}/{int(f_parc)}", valor_da_parcela, data_venc):
                            success_count += 1
                
                if success_count > 0:
                    st.success(f"✅ Registrado! {success_count} linhas enviadas para a planilha.")
                    time.sleep(1.5)
                    st.rerun()
                else:
                    st.error("Erro crítico de conexão com o Google. Verifique se o Forms está aceitando respostas.")
    ...
    if not df.empty:
        ...
        if pendentes.empty:
            st.success("Nenhum pagamento pendente no momento!")
        else:
            for i, r in pendentes.iterrows():
                with st.expander(f"{r['Cliente']} - {r['Tipo']} | R$ {r['Valor_Parc']}"):
                    if st.button("Confirmar Recebimento", key=f"bx_{i}"):
