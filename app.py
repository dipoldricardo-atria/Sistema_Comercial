...
st.set_page_config(page_title="ERP COMERCIAL 4.8.1", layout="wide", page_icon="🚀")

# SEU LINK DE SCRIPT ATUALIZADO
...
def carregar_dados():
    ...
    try:
        ...
        # Filtro de segurança para esconder o que foi apagado agora
        ...
        with st.form("login"):
            ...
            if st.form_submit_button("Acessar"):
                ...
                if not match.empty:
                    ...
                    st.rerun()
    except: st.error("Erro ao carregar banco de usuários.")
    st.stop()
...
if menu == "📝 Lançar & Gerir":
    ...
    with st.form("venda", clear_on_submit=True):
        ...
        if st.form_submit_button("🚀 GRAVAR NO BANCO"):
            ...
            status = []
            if f_parc == 0: 
                status.append(enviar_linha("À Vista", f_total, f_data))
            else:
                v_p = f_total / f_parc
    ...
    if u['cargo'] == "Admin":
        ...
        if not df_ex.empty:
            ...
            if sel != "Selecione..." and st.button("🔥 EXCLUIR DEFINITIVAMENTE", type="primary"):
                ...
                # Esconder da sessão imediatamente
